...
RemoteVideoTrackAdapter::~RemoteVideoTrackAdapter() {
  ...
  if (initialized()) {
    ...
    static_cast<MediaStreamRemoteVideoSource*>(web_track()->Source().GetPlatformSource())->OnSourceTerminated();
  }
}
...
void RemoteVideoTrackAdapter::InitializeWebVideoTrack(
    ...
    bool enabled) {
  DCHECK(main_thread_->BelongsToCurrentThread());
  auto video_source_ptr = std::make_unique<MediaStreamRemoteVideoSource>(std::move(observer));
...
void RemoteAudioTrackAdapter::InitializeWebAudioTrack(
  ...
  auto source = std::make_unique<PeerConnectionRemoteAudioSource>(observed_track().get(), main_thread);
