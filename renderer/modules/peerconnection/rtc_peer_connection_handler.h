...
class MODULES_EXPORT RTCPeerConnectionHandler {
 public:
  ...
  virtual void SetLocalDescription(blink::RTCVoidRequest* request, RTCSessionDescriptionPlatform* description);
  virtual void SetRemoteDescription(blink::RTCVoidRequest* request, RTCSessionDescriptionPlatform* description);
  ...
  virtual webrtc::RTCErrorType SetConfiguration(const webrtc::PeerConnectionInterface::RTCConfiguration& configuration);
  ...
  AddTransceiverWithTrack(const blink::WebMediaStreamTrack& web_track, const webrtc::RtpTransceiverInit& init);

  virtual webrtc::RTCErrorOr<std::unique_ptr<RTCRtpTransceiverPlatform>>
  AddTransceiverWithKind(const String& kind, const webrtc::RtpTransceiverInit& init);

  virtual webrtc::RTCErrorOr<std::unique_ptr<RTCRtpTransceiverPlatform>>
  AddTrack(const WebMediaStreamTrack& web_track, const Vector<WebMediaStream>& web_streams);
  ...
  // thread.
  void GetStandardStatsForTracker(scoped_refptr<webrtc::RTCStatsCollectorCallback> observer);
  void GetStats(webrtc::StatsObserver* observer, webrtc::PeerConnectionInterface::StatsOutputLevel level,
