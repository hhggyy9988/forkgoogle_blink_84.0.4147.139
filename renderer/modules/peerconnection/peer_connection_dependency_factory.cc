...
PeerConnectionDependencyFactory::PeerConnectionDependencyFactory(
  ...
  VLOG(1) << "hgy:obj=" <<this << " PeerConnectionDependencyFactory ctor";
  TryScheduleStunProbeTrial();
}

PeerConnectionDependencyFactory::~PeerConnectionDependencyFactory() {
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  VLOG(1) << "hgy:obj=" <<this << " PeerConnectionDependencyFactory dtor";
  DCHECK(!pc_factory_);
}
...
std::unique_ptr<RTCPeerConnectionHandler> PeerConnectionDependencyFactory::CreateRTCPeerConnectionHandler(
...
//    )");
  ...
  net::NetworkTrafficAnnotationTag traffic_annotation =
      net::DefineNetworkTrafficAnnotation("webrtc_peer_connection", "");
 
  socket_factory_.reset(new IpcPacketSocketFactory(p2p_socket_dispatcher_.get(), traffic_annotation));
  ...
  factory_options.disable_encryption = !blink::Platform::Current()->IsWebRtcEncryptionEnabled();
...
PeerConnectionDependencyFactory::CreateVideoTrackSourceProxy(
  ...
  return webrtc::VideoTrackSourceProxy::Create(signaling_thread_, worker_thread_, source).get();
}
