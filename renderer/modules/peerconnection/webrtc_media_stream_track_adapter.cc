...
// static
scoped_refptr<WebRtcMediaStreamTrackAdapter> WebRtcMediaStreamTrackAdapter::CreateLocalTrackAdapter(
  ...
  VLOG(1) << "hgy:static " << " CreateLocalTrackAdapter";
  scoped_refptr<WebRtcMediaStreamTrackAdapter> local_track_adapter(base::AdoptRef(new WebRtcMediaStreamTrackAdapter(factory, main_thread)));
  if (web_track.Source().GetType() == blink::WebMediaStreamSource::kTypeAudio) {
    local_track_adapter->InitializeLocalAudioTrack(web_track);
  } else {
    DCHECK_EQ(web_track.Source().GetType(), blink::WebMediaStreamSource::kTypeVideo);
...
// static
scoped_refptr<WebRtcMediaStreamTrackAdapter> WebRtcMediaStreamTrackAdapter::CreateRemoteTrackAdapter(
    blink::PeerConnectionDependencyFactory* factory,
    const scoped_refptr<base::SingleThreadTaskRunner>& main_thread,
  ...
  DCHECK(!main_thread->BelongsToCurrentThread());
  DCHECK(webrtc_track);
  VLOG(1) << "hgy:static " << " CreateRemoteTrackAdapter";
  scoped_refptr<WebRtcMediaStreamTrackAdapter> remote_track_adapter(base::AdoptRef(new WebRtcMediaStreamTrackAdapter(factory, main_thread)));
  if (webrtc_track->kind() == webrtc::MediaStreamTrackInterface::kAudioKind) {
    remote_track_adapter->InitializeRemoteAudioTrack(base::WrapRefCounted(static_cast<webrtc::AudioTrackInterface*>(webrtc_track.get())));
  } else {
    DCHECK_EQ(webrtc_track->kind(), webrtc::MediaStreamTrackInterface::kVideoKind);
    remote_track_adapter->InitializeRemoteVideoTrack(base::WrapRefCounted(static_cast<webrtc::VideoTrackInterface*>(webrtc_track.get())));
  }
...
void WebRtcMediaStreamTrackAdapter::InitializeLocalAudioTrack(
  ...
  local_track_audio_sink_.reset(new blink::WebRtcAudioSink(
      web_track_.Id().Utf8(), source_interface, factory_->GetWebRtcSignalingTaskRunner(), main_thread_));

  if (auto* media_stream_source = blink::ProcessedLocalAudioSource::From(blink::MediaStreamAudioSource::From(web_track_.Source()))) {
...
void WebRtcMediaStreamTrackAdapter::InitializeLocalVideoTrack(
  ...
  DCHECK_EQ(web_track.Source().GetType(), blink::WebMediaStreamSource::kTypeVideo);
  web_track_ = web_track;
  local_track_video_sink_.reset(new blink::MediaStreamVideoWebRtcSink(web_track_, factory_, main_thread_));
...
void WebRtcMediaStreamTrackAdapter::InitializeRemoteAudioTrack(
  ...
  VLOG(1) << "hgy:obj=" << this << " " << __FUNCTION__ << " E";
  SendLogMessage(base::StringPrintf("InitializeRemoteAudioTrack([this=%p])", this));
  remote_audio_track_adapter_ = base::MakeRefCounted<blink::RemoteAudioTrackAdapter>(main_thread_, webrtc_audio_track.get());
...
void WebRtcMediaStreamTrackAdapter::InitializeRemoteVideoTrack(
  ...
  remote_video_track_adapter_ =
      base::MakeRefCounted<blink::RemoteVideoTrackAdapter>(main_thread_, webrtc_video_track.get());
  webrtc_track_ = webrtc_video_track;
  remote_track_can_complete_initialization_.Signal();
  PostCrossThreadTask(
      *main_thread_.get(), FROM_HERE,
      CrossThreadBindOnce(&WebRtcMediaStreamTrackAdapter::
                              FinalizeRemoteTrackInitializationOnMainThread, WrapRefCounted(this)));
}

void WebRtcMediaStreamTrackAdapter::FinalizeRemoteTrackInitializationOnMainThread() {
  DCHECK(main_thread_->BelongsToCurrentThread());
