...
bool MediaStreamAudioSource::ConnectToTrack(
  ...
  mutable_blink_track.SetPlatformTrack(CreateMediaStreamAudioTrack(blink_track.Id().Utf8()));
  ...
  VLOG(1) << "Adding MediaStreamAudioTrack@" << track
...
MediaStreamAudioSource::CreateMediaStreamAudioTrack(const std::string& id) {
  ...
  return std::unique_ptr<MediaStreamAudioTrack>(new MediaStreamAudioTrack(is_local_source()));
}
